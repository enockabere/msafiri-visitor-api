...
def get_vendor_event_setups(
    ...
    for setup in setups:
        ...
        if setup.event_id:
            ...
            print(f"🏨 DEBUG: Query SQL: {allocations_query}")
            allocations = allocations_query.all()
            
            # Also check all allocations for this event to see what's there
            all_event_allocations = db.query(AccommodationAllocation).filter(
                AccommodationAllocation.event_id == setup.event_id,
                AccommodationAllocation.tenant_id == tenant_id
            ).all()
            
            print(f"🏨 DEBUG: All allocations for event {setup.event_id}: {len(all_event_allocations)}")
            for alloc in all_event_allocations:
                print(f"🏨 DEBUG: All Event Allocation {alloc.id}: guest={alloc.guest_name}, vendor_id={alloc.vendor_accommodation_id}, type={alloc.accommodation_type}, status={alloc.status}")
            
            print(f"🏨 DEBUG: Found {len(allocations)} vendor allocations for event {setup.event_id} and vendor {setup.vendor_accommodation_id}")
            for alloc in allocations:
                print(f"🏨 DEBUG: Filtered Allocation {alloc.id}: guest={alloc.guest_name}, vendor_id={alloc.vendor_accommodation_id}, status={alloc.status}")
            
            current_occupants = len(allocations)
            print(f"🏨 DEBUG: Final current_occupants: {current_occupants}")
            print(f"🏨 DEBUG: Setup vendor_accommodation_id: {setup.vendor_accommodation_id}")
            print(f"🏨 DEBUG: Setup event_id: {setup.event_id}")
