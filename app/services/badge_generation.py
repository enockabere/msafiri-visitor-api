...
def replace_template_variables(template_html: str, data: Dict[str, Any]) -> str:
    ...
    for key, value in variables.items():
        ...
        if key == 'logo' and value and value.startswith('http'):
            ...
            img_tag = f'<img src="{value}" alt="QR Code" style="width:72px;height:72px;margin:4px;background:white;display:block;object-fit:contain;border:1px solid #d1d5db" />'
            result = result.replace(f'{{{{{key}}}}}', img_tag)
            result = result.replace(f'{{{{{{{key}}}}}}}', img_tag)
...
async def generate_badge(
    ...
    try:
        ...
        if 'QR' in personalized_html and qr_code_url and not any(var in personalized_html for var in ['{{qr', '{{QR']):
            qr_img = f'<img src="{qr_code_url}" alt="QR Code" style="width:72px;height:72px;margin:4px;background:white;display:block;object-fit:contain;border:1px solid #d1d5db" />'
