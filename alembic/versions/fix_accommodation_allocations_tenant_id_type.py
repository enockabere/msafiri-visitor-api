...
Revision ID: fix_accommodation_allocations_tenant_id_type
...
revision = 'fix_accommodation_allocations_tenant_id_type'
down_revision = 'bec22112b245_add_accommodation_setup_id_to_events_'
branch_labels = None
depends_on = None
...
def reverse_table_tenant_id(table_name, index_name):
    """Helper function to reverse tenant_id type fix for a table"""
    # Step 1: Add temporary string column
    op.add_column(table_name, sa.Column('tenant_id_old', sa.String(length=50), nullable=True))
    
    # Step 2: Update with string values from tenants table
    op.execute(f"""
        UPDATE {table_name} 
        SET tenant_id_old = tenants.slug 
        FROM tenants 
        WHERE {table_name}.tenant_id = tenants.id
    """)
    
    # Step 3: Handle any unmapped records
    op.execute(f"""
        UPDATE {table_name} 
        SET tenant_id_old = 'default-tenant' 
        WHERE tenant_id_old IS NULL
    """)
    
    # Step 4: Drop the integer column and its index
    try:
        op.drop_index(index_name, table_name=table_name)
    except:
        pass
    ...
    # Step 5: Rename back to tenant_id
    op.alter_column(table_name, 'tenant_id_old', new_column_name='tenant_id')
    
    # Step 6: Make non-nullable
    op.alter_column(table_name, 'tenant_id', nullable=False)
...
def downgrade():
    # Reverse all the fixes
    reverse_table_tenant_id('vendor_event_accommodations', 'ix_vendor_event_accommodations_tenant_id')
    reverse_table_tenant_id('vendor_accommodations', 'ix_vendor_accommodations_tenant_id')
    reverse_table_tenant_id('rooms', 'ix_rooms_tenant_id')
    reverse_table_tenant_id('guesthouses', 'ix_guesthouses_tenant_id')
    reverse_table_tenant_id('accommodation_allocations', 'ix_accommodation_allocations_tenant_id')