...
Revision ID: 0144f8889f85
Revises: simple_add_app_feedback
Create Date: 2025-10-29 12:59:49.381513
...
def upgrade() -> None:
    ...
    op.alter_column('notifications', 'notification_type',
               existing_type=postgresql.ENUM('USER_CREATED', 'USER_ACTIVATED', 'USER_DEACTIVATED', 'ROLE_CHANGED', 'TENANT_CREATED', 'TENANT_ACTIVATED', 'TENANT_DEACTIVATED', 'VISITOR_INVITED', 'EVENT_CREATED', 'DOCUMENT_UPLOADED', 'SYSTEM_ANNOUNCEMENT', 'CHAT_MESSAGE', 'APP_FEEDBACK', name='notificationtype'),
               nullable=True)
...
def downgrade() -> None:
    ...
    sa.Column('category', postgresql.ENUM('health_program', 'security_briefing', 'events', 'reports', 'general', 'announcement', name='newscategory'), autoincrement=False, nullable=False),
    ...
    sa.ForeignKeyConstraint(['tenant_id'], ['tenants.id'], name=op.f('news_updates_tenant_id_fkey')),
    sa.PrimaryKeyConstraint('id', name=op.f('news_updates_pkey'))
    )
    ...
    sa.Column('updated_at', postgresql.TIMESTAMP(), server_default=sa.text('CURRENT_TIMESTAMP'), autoincrement=False, nullable=True),
    sa.ForeignKeyConstraint(['event_id'], ['events.id'], name=op.f('event_allocations_event_id_fkey'), ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['inventory_item_id'], ['inventory.id'], name=op.f('event_allocations_inventory_item_id_fkey'), ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['tenant_id'], ['tenants.id'], name=op.f('event_allocations_tenant_id_fkey'), ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('id', name=op.f('event_allocations_pkey'))
    )
