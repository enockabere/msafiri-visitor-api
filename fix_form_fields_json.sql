-- Fix field_options to be JSON strings instead of PostgreSQL arrays
-- First, delete the incorrectly formatted records
DELETE FROM form_fields WHERE field_options IS NOT NULL AND field_options::text LIKE '{%}';

-- Complete form fields initialization with proper JSON formatting
...
SELECT e.id, 'oc', 'Operational Center (OC)', 'select', 
'["OCA", "OCB", "OCBA", "OCG", "OCP", "WACA"]', 
true, 103, true, 'personal'
FROM events e
...
'["National Staff", "International Staff", "Consultant", "Volunteer"]', 
...
'["Fixed Term", "Permanent", "Temporary", "Project Based"]', 
...
'["Staying at venue", "Travelling daily"]', 
